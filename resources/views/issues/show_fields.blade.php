  ...
  <div class="col-md-3">
    <div class="form-group">
      ...
      <p>
        ...
        @if ($issues->status == 'RITADM') <span class='label label-danger'>Tiket diteruskan ke IT Operasional</span>
        @endif
        @if ($issues->status == 'RITSP') <span class='label label-danger'>Tiket diteruskan ke IT Operasional</span>
        @endif
        @if ($issues->status == 'AITSP') <span class='label label-warning'>Menunggu Tindakan Dari IT Support</span>
        @endif
        ...
        @if ($issues->status == 'SLITADM') <span class='label label-success'>Solusi Telah Diberikan IT
          Administrator</span> @endif
        @if ($issues->status == 'SLITOPS') <span class='label label-success'>Solusi Telah Diberikan IT OPS</span> @endif
        @if ($issues->status == 'SLITSP') <span class='label label-success'>Solusi Telah Diberikan IT Support</span>
        @endif
        ...
        @if ($issues->status == 'DLITADM') <span class='label label-warning'>Sedang Dalam Tindakan IT
          Administrator</span> @endif
        @if ($issues->status == 'DLITOPS') <span class='label label-warning'>Sedang Dalam Tindakan IT OPS</span> @endif
        @if ($issues->status == 'DLITSP') <span class='label label-warning'>Sedang Dalam Tindakan IT Support</span>
        @endif
...
<div class="row">
  <div class="col-md-4">
    <div class="form-group">
      ...
      <p>
        <input id="input-6" name="input-6" class="rating rating-loading" value="{{$issues->rating->rate??0}}"
          data-min="0" data-max="5" data-step="1" data-readonly="true">
      </p>
    </div>
  </div>
      ...
      <div class="modal-body">
        <div class="row">
          ...
          <div class="form-group col-sm-12 col-lg-6">
            ...
            {!! Form::select('assign_it_support',$it_support, null, ['class' => 'form-control select2',
            'style'=>'width:100%;']) !!}
          </div>
        </div>
      </div>
      ...
      <div class="modal-body">
        <div class="row">
          ...
          <div class="form-group col-sm-12 col-lg-6">
            ...
            {!! Form::select('assign_it_admin',$it_admin, null, ['class' => 'form-control select2',
            'style'=>'width:100%;']) !!}
          </div>
        </div>
      </div>
...
$issues->status == 'SLITADM' && $issues->assign_it_admin != null))
<!-- Button untuk IT NP -->
<div class="form-group col-md-2 col-sm-12">
...
@if ($issues->status == 'RT')
@if(($issues->assign_it_support != null && $issues->complete_by == $issues->assign_it_support)||($issues->assign_it_ops
!= null && $issues->complete_by == $issues->assign_it_ops))
<div class="form-group col-md-2 col-sm-12">
  {!! Form::open(['route' => ['issues.update', $issues->id], 'method' => 'patch']) !!}
  {!! Form::hidden('status', 'CLOSE', ['class' => 'form-control'])!!}
  <button class='btn btn-info btn-md' type="submit" onclick="return confirm('Yakin?')">
